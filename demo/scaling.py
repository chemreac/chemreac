...
def integrate_rd(tend=1.0, k1=7e-1, k2=3e2, k3=7.0,
                 A0=1.0, B0=0.0, C0=0.0, plot=False):
    def f(t):
    ...
    if plot:
        ...
        plt.subplot(3, 1, 2)
        #plt.plot(t, yout[:, 0, 0], label='A from integration')
        plt.plot(t, yout[:, 0, 0] - A_ssB,
                 label="A(t) for steady state for B")
        plt.legend()
        plt.subplot(3, 1, 3)
        plt.plot(t, yout[:, 0, 0] - A_ssB_2fast,
                 label="A(t) for ss B and k2 >> k3")
        plt.legend()
        plt.show()
    ydot = lambda x, y: (-k1*y[0] - k3*y[0]*y[1], k1*y[0] - k2*y[1], k2*y[1] + k3*y[0]*y[1])
