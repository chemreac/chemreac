    ...
    bin_k_factor_span(bin_k_factor_span), mode(mode)
{
    ...
    for (int rxni=0; rxni<nr; ++rxni){ // reaction index 
        ...
        for (int si=0; si<n; ++si){ // species index
            ...
            coeff_totl[rxni*n+si] = coeff_prod[rxni*n+si] -                coeff_reac[rxni*n+si];
        }
    }
...
#define FACTOR(ri, bi) (((ri) < n_factor_affected_k) ? 			bin_k_factor[bi][i_bin_k[ri]] : 1)
void
...
#define JAC(bri, bci, ri, ci) ja[((bri)*n + ri)*ldj+(bci)*n + ci]
void
ReactionDiffusion::dense_jac_rmaj(double t, const double * const restrict y,
        ...
        for (int si=0; si<n; ++si){
            ...
            for (int dsi=0; dsi<n; ++dsi){
                ...
                for (int rxni=0; rxni<nr; ++rxni){
                    ...
                    JAC(bi,bi,si,dsi) += coeff_totl[rxni*n + si]*                        coeff_actv[rxni*n + dsi]*local_r[rxni]/C[dsi];
                }
            }
        }
...
#undef JAC
#define JAC(bri, bci, ri, ci) ja[((bci)*n + ci)*ldj+(bri)*n + ri]
void
ReactionDiffusion::dense_jac_cmaj(double t, const double * const restrict y,
                            double * const restrict ja, int ldj) const
{
...
#undef JAC
#define JAC(bri, bci, ri, ci) ja[((bci)*n + ci)*ldj+2*n+(bri)*n+ri-((bci)*n+ci)]
void
ReactionDiffusion::banded_jac_cmaj(double t, const double * const restrict y,
                            double * const restrict ja, int ldj) const
{
...
#undef JAC
#define JAC(bri, bci, ri, ci) ja[((bci)*n+ci)*ldj+n+(bri)*n+ri-(bci)*n-ci]
void
ReactionDiffusion::banded_packed_jac_cmaj(double t, const double * const restrict y,
                            double * const restrict ja, int ldj) const
{
