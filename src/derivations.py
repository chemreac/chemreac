...
def finite_diff(x, y, i, offset=0, dx_=None, dy_=None):
    ...
    of of y wrt x from three arbitrarily spaced points:
        [(x[i-1], y[i-1]), (x[i], y[i]), (x[i+1], y[i+1])]
    either at:
      the left point (offset=-1) or at
      the central point (offset=0) or at
      the right point (offset=1)

    the separations between the points are:

    dx_ is an optional callback creating delta x terms for index i
    """
    ...
    return 2*a_*(x[i+offset]-x[i])+b_, 2*a_
