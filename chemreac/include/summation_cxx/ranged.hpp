...
#else
#define SMMTNCXX_OUTPUT(OP) this->tgt[i] OP (*this)[i].template to<target_type>();
#endif
#define SMMTNCXX_LOOP std::size_t i = 0; i < this->sz; ++i
            if (cumulative) {
              for (SMMTNCXX_LOOP) {
                SMMTNCXX_OUTPUT(+=)
              }
...
#undef SMMTNCXX_LOOP
#undef SMMTNCXX_OUTPUT
        }
    };
    template<typename T, typename U=void> using RangedAccumulatorKahan = RangedAccumulator<T, Compensation::KAHAN, U>;
    template<typename T, typename U=void> using RangedAccumulatorNeumaier = RangedAccumulator<T, Compensation::NEUMAIER, U>;
    ...
    public:
        ...
        RangedUncompensatedView(std::size_t sz) : sz(sz) {}
        void init(target_type * target, bool cumulative=false) {
            tgt = target;
