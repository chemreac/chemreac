...
cdef class PyReactionDiffusion:
    ...
    property k:
        def __get__(self):
            cdef cnp.ndarray[cnp.float64_t, ndim=1] k_ = np.empty(self.thisptr.nr)
            self.thisptr.get_k(<double*>k_.data)
            return k_
        def __set__(self, vector[double] k_):
            assert len(k_) == self.nr
            self.thisptr.set_k(&k_[0])
