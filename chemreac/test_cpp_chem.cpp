...
int main(){
  ...
  int ntimings = 10;
  int n = 4;
  int N = 200000000; // A ridiculous number of bins for 1D but used for benchmarking
  vector<vector<int> > stoich_reac {{0}, {1, 2, 2}};
  vector<vector<int> > stoich_prod {{1}, {1, 3}};
  vector<double> k {0.05, 3.0};
  vector<double> D {.1, .2, .3, .4};
  ReactionDiffusion rd(4, 10, stoich_reac, stoich_prod, k, D, 0);
  vector<double> y;
  vector<double> b;
  ...
  for (auto i = 0; i < ntimings; ++i){
    ...
    int info = rd.banded_jaclu_solve(t, &y[0], &b[0], 1, 1.0);
    assert(info == 0);
